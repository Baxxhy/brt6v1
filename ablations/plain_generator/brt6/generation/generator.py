...
from ..core.joint_reference import joint_context
...
def generate_candidate(
    ...
    seed_test_code = _prompt_text(
        current_test_code
        or "No prior candidate. Generate from the shared Top-3 collection below.",
    ...
    user_prompt = render_evidence_prompt(user_prompt, behavior) + joint_context(related_test)
    if protocol is not None:
        user_prompt += (
            "\n\nRequired test protocol to preserve:\n"
            + _prompt_json(protocol.to_dict(), MAX_PROMPT_PROTOCOL_CHARS)
        )
    assert semantic_delta is None and not delta_history, "Planning disabled in this ablation"
