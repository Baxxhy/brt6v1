...
class AblationConfig:
    ...
    def validate(self) -> "AblationConfig":
        ...
        if set(disabled) in ({"mutation", "semantic_delta"}, {"behavior_target", "mutation", "semantic_delta"}):
            return self
        if len(disabled) > 1:
            raise ValueError(
    ...
    def ablation_id(self) -> str:
        if not self.mutation and not self.semantic_delta:
            return "wo_individual_adaptation" if self.behavior_target else "wo_individual_adaptation_direct_fallback"
    ...
    def method_variant(self) -> str:
        if not self.mutation and not self.semantic_delta:
            return "w/o Individual Adaptation (joint references, no Delta)"
        if not self.disabled_component:
            return "full"
    ...
    def signature(self) -> str:
        ...
        signature += ";direct_fallback={}".format(
            int(self.behavior_target)
        )
        if not self.mutation:
            signature += ";seed_generation_mode=joint_top3_three_draws_v1"
        return signature + ";reference_mode=joint_top3_v1"

    def to_dict(self) -> dict[str, Any]:
        flags = {name: bool(getattr(self, name)) for name in _COMPONENTS}
        return {
            **flags,
            "individual_adaptation": False,
            "reference_mode": "joint_top3_v1",
            ...
            "direct_fallback": bool(self.behavior_target),
            "seed_generation_mode": (
                "validated_plan_per_seed"
                if self.mutation
                else "joint_context_generation_per_draw"
            ),
