...
from ..context.host_context import build_host_context, rank_related_tests as _original_rank_related_tests, select_related_test
from ..core.joint_reference import joint_rank

def rank_related_tests(tests, behavior):
    return joint_rank(_original_rank_related_tests, tests, behavior)
...
def _should_run_direct_fallback(
    ...
    if os.environ.get("BRT_DISABLE_DIRECT_FALLBACK") == "1":
        return False
    if config.ablation_id not in {"full", "wo_individual_adaptation"}:
        return False
    return not any(
...
def _semantic_feedback_payload(
    ...
    """Ordinary execution feedback, without a semantic repair plan."""
    return {
        "decision": decision.decision,
        "reason": decision.reason,
        "failure_class": str(getattr(strict_result, "failure_class", "")),
    }
...
def run_instance_pipeline(
    ...
    try:
        ...
        oracle_type = ""
        initial_delta = None
        if True:  # Direct joint-context generation, without a Delta planner.
        ...
        while brt_attempt < max_brt_attempts:
            ...
            if next_round >= max_brt_attempts:
                break
            delta = None  # Direct feedback repair; no planning request.
            candidate = generate_candidate(
                context.instance_id,
                behavior,
                host,
                ...
                context.buggy_repo_path,
                next_round,
                feedback=json.dumps({"verifier": semantic_feedback, "actual_execution": execution.stdout + "\n" + execution.stderr}, ensure_ascii=False),
                write_to_repo=True,
                protocol=protocol,
