...
def _environment_spec_cache_path(url: str) -> Path:
    root = Path(
        os.environ.get(
            "BRT4_ENV_SPEC_CACHE_DIR",
            str(Path.home() / ".cache" / "brt5" / "environment_specs"),
        )
    )
    return root / (hashlib.sha256(url.encode("utf-8")).hexdigest() + ".txt")


def _cached_response(path: Path):
...
def raw_get(url: str):
    ...
    for candidate in urls:
        cache_path = _environment_spec_cache_path(candidate)
        ...
        with lock_path.open("w", encoding="utf-8") as lock:
            ...
            try:
                ...
                last_exc = RuntimeError(
                    f"{candidate} returned status {response.status_code}"
                )
